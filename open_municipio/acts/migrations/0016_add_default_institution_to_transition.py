...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        all_transitions = orm.Transition.objects.all()

        # add the proper owner to all existing Transition's
        for curr_tr in all_transitions:
            if curr_tr.act:
                curr_tr.institution = curr_tr.act.emitting_institution
                curr_tr.save()

        # check that no Transition is left without owner
        err_transitions = orm.Transition.objects.filter(institution=None)
        
        assert len(err_transitions) == 0
