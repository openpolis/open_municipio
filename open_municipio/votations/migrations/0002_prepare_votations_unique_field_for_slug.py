...
class Migration(DataMigration):
    ...
    votation_unique_fields = [ "sitting", "idnum" ]

    def forwards(self, orm):

        dup_votations = self.get_dup_votations(orm)

        msg_votations = ""
        if len(dup_votations) > 0:

            msg_acts = "Non unique votations for the following parameters: %s" % dup_votations

        if msg_votations:

            msg = "Resolve manually this inconsistency, before re-running this migration"

            raise RuntimeError("%s. %s." % (msg_votations, msg))
    ...
    def get_dup_votations(self, orm):
        
        duplicates = orm.Votation.objects.all().values(*self.votation_unique_fields).annotate(_num_dup=models.Count("pk")).filter(_num_dup__gt=1)
